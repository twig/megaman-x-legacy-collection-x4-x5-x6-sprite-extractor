# https://github.com/Kuumba123/MMX5---X6-DAT-Extract/blob/main/datExtract.py
...
def process_arc_file(input_file_path, output_directory, baseName):
    ...
    while sector != 0:
        ...
        arc_size = int.from_bytes(
            data[sector * 0x800 + 4 : sector * 0x800 + 7], byteorder="little"
        )

        if arc_size != size:
            file_name = f"{baseName}_{fileId}.BIN"
        else:
            file_name = f"{baseName}_{fileId}.ARC"
...
if len(sys.argv) < 3:
    ...
    print(
        "This Program is used for extracting MegaMan X5/X6 DAT files into ARC/BIN files"
    )
